module hdf5_wrapper
  ...
  public :: hdf5_get_dimensions
  public :: hdf5_get_type
  public :: hdf5_create_group
  public :: hdf5_list_datasets
  public :: hdf5_list_groups
  public :: hdf5_list_attributes
  public :: hdf5_get_file_id
  ...
  !
  ! Everything below here is private and internal to the hdf5 wrapper module
  ! --------------------------------------------------------------------------
  !
  
  ! maximum number of files which can be open
  integer, parameter :: nfilemax = 1000

  ! table of currently open files
  integer :: nopen = 0
  integer(hid_t), dimension(nfilemax) :: file_id
  logical, dimension(nfilemax) :: read_only
  ...
  integer(hid_t) :: NATIVE_INTEGER4
  integer(hid_t) :: NATIVE_INTEGER8
  ...
  integer(hid_t) :: NATIVE_REAL4
  integer(hid_t) :: NATIVE_REAL8

  ! Information about the currently open dataset
  logical             :: open_dset
  integer(hid_t)      :: open_dset_id
  character(len=1000) :: open_file_name, open_dset_name
...
!
  ...
  subroutine hdf5_close_file(ifile)
    ...
    if(HDF_VERBOSITY .ge. 1)then
       ! Check if we left any objects open
       write(0,*)"[hdf5_close_file] Closing file..."
       call h5fget_obj_count_f(file_id(ifile), H5F_OBJ_ALL_F, obj_count, &
            hdf_err)
       write(0,*)"[hdf5_close_file] Total objects open: ", obj_count
       call h5fget_obj_count_f(file_id(ifile), H5F_OBJ_GROUP_F, obj_count, &
            hdf_err)
       write(0,*)"[hdf5_close_file] Groups open: ", obj_count
       call h5fget_obj_count_f(file_id(ifile),H5F_OBJ_DATASET_F,obj_count, &
            hdf_err)
       write(0,*)"[hdf5_close_file] Datasets open: ", obj_count
       call h5fget_obj_count_f(file_id(ifile),H5F_OBJ_FILE_F,obj_count, &
            hdf_err)
       write(0,*)"[hdf5_close_file] Files open: ", obj_count
    endif
...
!
    ...
    integer(hid_t) :: dset_id, attr_id, loc_id, dtype_id
    integer :: hdf_err, hdf_err2
    logical :: isattribute, isgroup
    integer :: nslash
    character(len=200) :: loc_name,attr_name
